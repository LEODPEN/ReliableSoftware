...
g \in 0 \upto n-1 \rightarrow N,\exists x \cdot x \in k \upto n-1 \wedge g(l)=g(x) \vdash l = x \\
...
PRE, k!=n-1,\forall i,j \cdot i \in 0 \upto k-1 \wedge j \in i+1 \upto n-1 \Rightarrow g(i)<g(j) \vdash [l:=g^{-1}(min(g[k\upto n-1])),g:=(\{k,l\} \domsub g) \cup {k \mapsto g(l),l \mapsto g(k)},k:=k+1]\forall i,j \cdot i \in 0 \upto k-1 \wedge j \in i+1 \upto n-1 \Rightarrow g(i)<g(j) \\
